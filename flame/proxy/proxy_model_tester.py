"""Run a whole proxy analysis locally, one thread per simulated node.
...
from flame.proxy import Proxy, ProxyAggregator, ProxyAnalyzer, ProxyModel
...
_SEED_NODE_ID = "da275317-dcdf-4206-2e5c-2d7a6d0bf320"
...
class ProxyModelTester:
    """Simulates a full proxy analysis in threads and writes out its result.

    One thread is started per data split (the analyzers), plus ``num_proxy_nodes``
    proxies and one aggregator. All threads share the mock SDK's class-level
    message broker, so they communicate exactly as the real nodes would - the
    role self-reporting and the analyzer-to-proxy assignment included.

    As with :class:`~flame.proxy.proxy_model.ProxyModel`, constructing the class
    ...
    def __init__(
        ...
        :param analyzer: Subclass of
            :class:`~flame.proxy.analyzer_client.Analyzer` to run on each
            analyzer node.
        :param proxy: Subclass of :class:`~flame.proxy.proxy_client.Proxy` to
            run on each proxy node.
        :param aggregator: Subclass of
            :class:`~flame.proxy.aggregator_client.Aggregator` to run on the
            aggregator node.
        :param data_type: Which data source the splits stand in for.
        :param node_roles: Explicit role per data split; defaults to "default"
            for every split.
        :param query: FHIR query/queries or S3 key(s) to fetch from the mock.
        :param num_proxy_nodes: How many proxy nodes to simulate.
        ...
        # Derive one deterministic uuid per node. Nodes are laid out as
        # [analyzers..., proxies..., aggregator], the aggregator getting a fixed
        # offset from the first id.
        ...
        for i, participant in enumerate(participants):
            ...
            # kwargs is bound as a default so each thread captures its own
            # values rather than the last ones seen by the loop.
